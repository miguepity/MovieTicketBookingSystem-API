...
export class ReembolsosController {
  ...
  @ApiResponse({
    status: 403,
    description: 'Acceso denegado. Se requiere rol ADMIN o RECEPCIONISTA.',
  })
  @ApiResponse({
    status: 404,
    description: 'El pago con el ID proporcionado no existe.',
  ...
  @ApiOperation({
    summary:
      'Registrar reembolso manual en efectivo y notificar al usuario que lo procesa',
  })
  ...
  async registrarReembolsoEfectivo(
    ...
    return await this.reembolsosService.registrarReembolsoEfectivo(
      dto,
      req.user.email,
    );
  }
